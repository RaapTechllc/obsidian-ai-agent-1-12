...
def _note_to_info(
    note: Note, vault_manager: VaultManager, response_format: str, relevance: float = 1.0
) -> NoteInfo:
    """Convert Note to NoteInfo based on response format.

    Args:
        note: Note object from VaultManager.
        vault_manager: VaultManager instance for path calculations.
...
async def execute_semantic_search(
    ...
    note_infos = [_note_to_info(n, vault_manager, response_format) for n in notes]
...
async def execute_find_related(
    ...
    # Convert to NoteInfo
    note_infos = [_note_to_info(n, vault_manager, response_format) for n in related_notes]
