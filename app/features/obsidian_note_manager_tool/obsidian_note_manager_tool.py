...
async def obsidian_note_manager_tool(
    ...
    criteria: BulkOperationCriteria | None = None,
...
) -> ObsidianNoteManagerToolResult | BulkOperationPreview:
    ...
    Args:
        operation: Type of modification operation to perform.
            ...
            - "bulk_find_and_tag": Find notes matching criteria and add tags (requires criteria, metadata with "tags" key)
                Use when: User wants to "find all notes about topic and tag them"
            - "bulk_find_and_move": Find notes matching criteria and move them (requires criteria, destination_folder; optional create_folders)
                Use when: User wants to "find all project notes and organize them"
            - "bulk_preview": Preview what a bulk operation will affect (requires criteria)
                Use when: User wants to "preview what will change" before applying operation
        ...
        criteria: Search criteria for find operations.
            Example: {"tags": ["project"], "content_contains": "AI", "folder_path": "Daily"}
            Required for: bulk_find_and_tag, bulk_find_and_move, bulk_preview
        destination_folder: Folder path for bulk_move/bulk_find_and_move operation.
            Example: "Archive/2024" - all notes will be moved here
            Required for: bulk_move, bulk_find_and_move
        changes: Changes to preview in bulk_preview operation.
            Example: {"tags": ["reviewed"]} - tags that would be added
            Used for: bulk_preview
    ...
    try:
        ...
        elif operation == "bulk_find_and_tag":
            if not criteria or not metadata:
                raise ValueError("bulk_find_and_tag requires criteria and metadata parameters")
            result = await service.execute_bulk_find_and_tag(vault_manager, criteria, metadata)

        elif operation == "bulk_find_and_move":
            if not criteria or not destination_folder:
                raise ValueError("bulk_find_and_move requires criteria and destination_folder parameters")
            result = await service.execute_bulk_find_and_move(
                vault_manager, criteria, destination_folder, create_folders
            )

        elif operation == "bulk_preview":
            if not criteria:
                raise ValueError("bulk_preview requires criteria parameters")
            result = await service.execute_bulk_preview(
                vault_manager, operation, criteria, changes, destination_folder
            )
